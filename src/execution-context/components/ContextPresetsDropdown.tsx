...
import { CONTEXT_PRESETS } from '../engine/presets';
import { ContextPreset } from '../engine/types';

interface ContextPresetsDropdownProps {
  currentPresetId: string | null;
  onSelectPreset: (preset: ContextPreset) => void;
}

function ContextPresetsDropdown({
  currentPresetId,
  onSelectPreset,
}: ContextPresetsDropdownProps) {
  ...
  const activePreset = CONTEXT_PRESETS.find((p) => p.id === currentPresetId);
        ...
        <div className="absolute left-0 sm:right-0 sm:left-auto top-full mt-1.5 w-80 sm:w-96 rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface-elevated)] p-2 shadow-2xl z-50 animate-in fade-in zoom-in-95">
          <div className="flex items-center justify-between px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] border-b border-[var(--border-subtle)]">
            <div className="flex items-center gap-1">
              ...
              <span>Execution Context Examples</span>
            </div>
            <span>{CONTEXT_PRESETS.length} presets</span>
          </div>
          ...
          <div className="max-h-80 overflow-y-auto space-y-1 p-1">
            {CONTEXT_PRESETS.map((preset) => {
              ...
              if (preset.category === 'Hoisting') {
                categoryClass =
                  'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20';
              } else if (preset.category === 'Functions') {
                categoryClass =
...
export default memo(ContextPresetsDropdown);
