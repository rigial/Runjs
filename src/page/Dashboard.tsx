...
function Dashboard() {
    ...
    async function dbcall() {
        try {
            ...
            const filterIsnotDeleted = dbResult.filter(val => val.isDelete === false)
            const tagHashMap: Tag = {};
            filterIsnotDeleted.forEach((val) => (tagHashMap[val.tag] = (tagHashMap[val.tag] ?? 0) + 1));
    ...
    function filterList() {
        ...
        const filter = userSavedCode.filter(val => val.fileName.toLowerCase().includes(searchTerm) || val.tag.toLowerCase().includes(searchTerm));
        return setFilterData(filter);
    }
    ...
    return (
        <Fragment>
            <section className="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
                ...
                <SearchInput showFavourite={filterData.length === 0} isFavouriteSelected={isFavouriteSelected} setIsFavouriteSelected={setIsFavouriteSelected} searchTerm={searchTerm} dialogRef={dialogRef} onInputChange={onInputChange} />
                {Object.keys(tag).length > 0 ? <section className="flex flex-wrap gap-2 mt-3">
                    <h1>Your Tags : </h1>
                    {Object.keys(tag).map((val, index) => (
                        <Badge searchTerm={searchTerm} setSearchTerm={setSearchTerm} key={index} tag={val} count={tag[val]} />
                    ))}
                </section> : null}
                <ProjectTable tagSuggestions={Object.keys(tag)} dbcall={dbcall} data={filterData} bin={false} createPlayground={() => dialogRef?.current?.open()} />
            </section>
            <CreatePlayground edit={false} ref={dialogRef} tagSuggestions={Object.keys(tag)} />
        </Fragment>
    )
}
