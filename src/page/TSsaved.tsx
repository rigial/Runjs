...
function TSsaved() {
    ...
    async function handleRunClick() {
        ...
        if (consoleRef.current) {
            ...
            try {
                const parseJavascriptCode = await transform(code?.code ?? "", { loader: 'ts' });
        ...
        <Fragment>
            <main className='bg-CustomDarkGrey'>
                <nav className='h-7vh w-full flex items-center justify-between px-3'>
                    ...
                    <button onClick={handleRunClick} className={`bg-TSBlue text-black font-semibold py-1 px-2 rounded flex flex-row gap-1 items-center hover:cursor-pointer`}>
                        ...
                        <section className="h-full w-full flex flex-col">
                            <div className="flex overflow-x-auto border-[1px] border-WindowBorder">
                                <div className={`flex items-center gap-2 bg-cblack py-2 px-3 border-r-[1px] border-r-WindowBorder hover:cursor-pointer`}>
                                    <img src={"/Typescript.webp"} className="w-5 h-5 rounded-sm" alt="Typescript Logo" />
