...
import { IModalProps, ModalRef, UserCodeBase } from "../utils/interface";
import { addCode, updateCode } from "../db/operations";
import { useNavigate } from "react-router-dom";


const CreatePlayground = forwardRef<ModalRef, IModalProps>(({ tagSuggestions, edit, renameData, dbcall }, ref) => {
    ...
    return (
        <dialog
            ...
            <h2 className="text-lg font-semibold text-gray-800">{edit ? renameData?.fileName : "Create A Playground"}</h2>
            <p className="mt-2 text-sm text-gray-600">{edit ? "Enter a new title for this playground:" : "Give it a nice name like"}</p>
            <form className="mt-4 flex flex-col justify-end" onSubmit={createNewPlayGroundFunction}>
                <input
                    autoFocus={true}
                    maxLength={50}
                ...
                {!edit && <div className="flex w-full gap-4 justify-center mt-3">
                    <button onClick={() => setLang("js")} type="button" className={`bg-transparent p-1 border ${lang === "js" ? "border-black" : "border-gray-200"} rounded`}>
                        <img src="/JavaScript.webp" className="size-9" alt="Javascript Logo" />
                    </button>
                    <button onClick={() => setLang("ts")} type="button" className={`bg-transparent p-1 border ${lang === "ts" ? "border-black" : "border-gray-200"} rounded`}>
                        <img src="/Typescript.webp" className="size-9" alt="Typescript Logo" />
                    </button>
                </div>}
                <button
                    type="submit"
