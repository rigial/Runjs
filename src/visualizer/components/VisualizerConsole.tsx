...
import { ConsoleLogEntry } from '../engine/types';

interface VisualizerConsoleProps {
  logs: ConsoleLogEntry[];
  onClear?: () => void;
}

function VisualizerConsole({ logs, onClear }: VisualizerConsoleProps) {
  return (
    <div className="flex flex-col h-full rounded-xl border border-[var(--border-default)] bg-[var(--bg-surface)] overflow-hidden shadow-xs">
      ...
      {/* Log Entries */}
      <div className="flex-1 p-3 overflow-y-auto font-mono text-xs flex flex-col gap-1.5 bg-[var(--bg-app)]">
        {logs.length === 0 ? (
...
export default memo(VisualizerConsole);
